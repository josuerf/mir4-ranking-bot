    ...
    run: async(client, message, args) => {
        const getErrors = (info) => {
            const errorMsgs = {
                ...
                mClan: '- O formato do clan informado não é suportado'
            }
            const errors = []
        ...
        const mClan = message.content.match(/(?<=[Cc]lan:[\s\S]).+/g)

        const errors = getErrors({ mNick, mLevel, mPower, mClan })
        ...
        fs.writeFileSync('./src/ranking.json', JSON.stringify(ranking, null, 4))
    }
}
